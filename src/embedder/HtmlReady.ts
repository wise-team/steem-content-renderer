...
*/
export default function(html: string, { mutate = true, hideImages = false } = {}) {
    ...
    try {
        ...
        // xmldom error is bad
        Log.log().warn("rendering error", JSON.stringify({ error: error.message, html }));
        return { html: "" };
    }
}

function traverse(node: ChildNode | Document, state: any, depth = 0) {
...
function link(state: any, child: Node) {
...
function iframe(state: any, child: any) {
...
function img(state: any, child: any) {
    const url = child.getAttribute("src");
    if (url) {
...
function proxifyImages(doc: Document) {
    if (!doc) {
        return;
    }
...
function linkify(content: string, mutate: any, hashtags: any, usertags: any, images: any, links: any) {
    // hashtag
...
function embedYouTubeNode(child: any, links: any, images: any) {
    try {
...
function embedVimeoNode(child: any, links: any /*images*/) {
    try {
        const data = child.data;
...
function embedTwitchNode(child: any, links: any /*images*/) {
    try {
        const data = child.data;
...
function twitchId(data: any) {
    ...
    return {
        ...
        url: m[0],
        canonical:
            m[1] === `videos` ? `https://player.twitch.tv/?video=${m[2]}` : `https://player.twitch.tv/?channel=${m[2]}`,
    };
}

function ipfsPrefix(url: any) {
