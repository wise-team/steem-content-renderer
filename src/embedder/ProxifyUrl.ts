/**
 * Based on:
 * https://github.com/steemit/condenser/blob/425656e260b2920d5250e57f5b6ac37ccf050a97/src/app/utils/ProxifyUrl.js
 */
 ...
 * @param {string} url
 * @param {string|boolean} dimensions - optional -  if provided. url is proxied && proxy prefix is avail. resp will be
 * "{proxy prefix}{dimensions}/{sanitized url}"
 * if falsy, all proxies are stripped.
 * if true, preserves the first {int}x{int} in a proxy url. If not found, uses 0x0
 * @returns string
 */
export default (url: string, dimensions: string | boolean = false) => {
