...
import { getExternalLinkWarningMessage, getPhishingWarningMessage } from "shared/HtmlReady"; // the only allowable title attributes for div and a tags
...
export default ({ large = true, highQualityPost = true, noImage = false, sanitizeErrors = [] }) => ({
    ...
    transformTags: {
        iframe: (tagName: string, attribs: string) => {
